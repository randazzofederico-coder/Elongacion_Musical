...
- **Dynamic Analytical Metronome:** C++ Math-driven software oscillator. No pre-rendered `.wav` files required. Dart passes complex 2D arrays (N-Pulses x M-Subdivisions) flattened via FFI pointers dynamically. Each instance tracks its own custom sequencer grids (e.g., 5/4 alongside 4/4). It features a robust text-input parsing system with a tailored Virtual Keyboard allowing users to define asymmetric structures (e.g., `3+2`), fractional subdivisions (e.g., `1/3+2`), and **tuplet notation** (e.g., `3:2`, `2:3/3`) instantly.
  - **Tuplet Notation (`A:B`):** Means "A pulses in the time B normal beats". Each pulse gets `durationRatio = B/A`. For example, `3:2` creates 3 pulses with `durationRatio = 0.667` each (total cycle = 2 beats). `1:3` creates 1 pulse spanning 3 beats. Full syntax per segment: `count[:ratio][/subdivisions]`, combined with `+` for compound patterns (e.g., `3:2/6+2`).
  - **Duration-Aware Scheduling (C++):** The `MetronomePulse` struct stores a `durationRatio` field. The `_mixInternal` scheduling replaces simple `beat % numPulses` modulo logic with cumulative duration-based positioning: the cycle length = `sum(durationRatio[i])`, and pulse boundaries are computed from accumulated ratio sums via a `findPulseAndSub` lambda that maps a continuous beat position to the correct pulse and subdivision index.
  - **Full-Stack API:** `durationRatios` are passed as `double*` (C++), `Pointer<Double>` (Dart FFI), and `Float64List` → `HEAPF64` (Web/WASM) through all layers: `live_mixer.h/cpp` → `live_mixer_bindings.dart` → `live_mixer_native.dart` / `live_mixer_web.dart` → `audio_worklet_processor.js`.
  - **Formatted Structure Display:** When not editing, the structure string renders as a fraction-like widget: top part (count or count:ratio) above a horizontal divider, subdivision number below. Segments separated by `+` show inline with `+` symbols between them. When editing, falls back to the raw text field with custom keyboard (now including the `:` key).
  - **Proportional Sequencer Cells:** Both the per-pattern sequencer and the macro cycle visualizer use `Expanded(flex: durationRatio * 100)` so tuplet pulses display at their correct relative widths — ensuring visual alignment across birritmia patterns.
- **Micro & Macro Visualization:** Uses a "Split-Cell" UI for subdivisions, maintaining a linear timeline by intelligently dividing pulse containers. The `MetronomeProvider` calculates the Least Common Multiple (LCM) of all instances' cycle durations (`sum(durationRatio)` per instance) to establish a unified "Macro Cycle". Each instance's pattern repeats to fill the macro cycle, with cell widths proportional to `durationRatio`. A minimalist UI `Ticker` polls the C++ atomic engine frame-counter at 60fps to accurately trace playback progress across a miniaturized, anatomically correct global cycle bar.
  - **Default Birritmia:** App launches with two patterns demonstrating 3/4 vs 6/8 polyrhythm: Pattern 1 `3/2` = `[0,0,2,0,2,0]` (3 pulses × 2 subdivisions, durationRatio 1.0 each), Pattern 2 `2:3/3` = `[1,0,0,1,0,0]` (2 pulses × 3 subdivisions, durationRatio 1.5 each). Both sum to 3 beats, producing aligned 6-subdivision rows in the macro visualizer.
